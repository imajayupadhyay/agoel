...
class HomepageManager
{
    public function __construct(
        private readonly HomepageSchema $schema,
        private readonly HomepageMedia $media,
    ) {}

    public function update(Page $page, UpdateHomepageRequest $request): void
    {
        DB::transaction(function () use ($page, $request): void {
            ...
            foreach ($request->validated('sections', []) as $sectionId => $submitted) {
                ...
                $section->update([
                    'name' => $section->is_custom
                        ? $this->clean($submitted['name'] ?? $section->name)
                        : $section->name,
                    'content' => $content,
